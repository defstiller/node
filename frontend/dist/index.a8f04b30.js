...
const jobSearch = new (0, _jobSearch.JobSearch)("#search-form", "#results-container", "#loading-element");
jobSearch.setCountryCode();
jobSearch.setFormListener();

},{"./JobSearch":"7jUiC"}],"7jUiC":[function(require,module,exports) {
...
class JobSearch {
    ...
    setCountryCode() {
        fetch("http://ip-api.com/json").then((response)=>response.json()).then((data)=>this.countryCode = data.countryCode.toLowerCase());
    }
    setFormListener() {
        this.searchForm.addEventListener("submit", (event)=>{
            ...
            const searchParams = {};
            for (const [key, value] of formData.entries())searchParams[key] = value;
            ...
            fetch(`http://localhost:3000/?search=${search}&location=${location}&country=${this.countryCode}`).then((response)=>response.json()).then(({ results  })=>{
                this.stopLoading();
                console.log(results);
                results = results.map((result)=>(0, _jobCard.jobCard)(result)).join("");
...
},{"./jobCard":"8EpWn","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"8EpWn":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "jobCard", ()=>jobCard);
...
},{}]},["dAPrZ","5AKj5"], "5AKj5", "parcelRequire10c2")
